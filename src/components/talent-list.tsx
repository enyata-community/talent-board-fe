  ...
  return (
    <div className="mt-5">
      ...
      {isLoading ? (
        ...
        <>
          {allTalents.length === 0 && (
            <div className="flex flex-col items-center justify-center gap-2 p-12 text-center min-h-[400px]">
              <UserSearch className="size-6 text-muted-foreground/50" />
          ...
          <div className="mt-2 grid md:grid-cols-2 grid-cols-1 gap-3">
            {allTalents.map((talent: talentProp, i: number) => (
              <TalentCard key={i} talent={talent} width="max-w-full" />
            ))}
          </div>
        </>
      )}
